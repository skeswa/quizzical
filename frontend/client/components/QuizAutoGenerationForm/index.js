...
class QuizAutoGenerationForm extends Component {
  static propTypes = {
  ...
  componentDidMount() {
    const { categories, genStratParam } = this.props
  ...
  render() {
    const { loading, categories, outsidePopup, genStratParam } = this.props
    const {
      quizLength,
      generationStrategy,
      selectedCategoryId,
    ...
    const quizLengthSliderWrapperStyle = {
      height: genStratParam === GENERATION_STRATEGY_BY_WEAKNESS
...
export default QuizAutoGenerationForm
