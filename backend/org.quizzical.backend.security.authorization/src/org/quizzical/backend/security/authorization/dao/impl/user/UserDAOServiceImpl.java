package org.quizzical.backend.security.authorization.dao.impl.user;
...
import org.quizzical.backend.security.authorization.dao.impl.user.email.WelcomeMessagePreparator;
import org.quizzical.backend.security.authorization.model.jpa.user.JPAUser;
...
public class UserDAOServiceImpl extends BaseServiceImpl implements IUserDAOService {
	...
	@Override
	public User requiresDiagnostic(User record) throws ApplicationException {
		User usr = getByEmail(record.getEmailAddress());
		usr.setRequiresDiagnosticTest(true);
		return update(usr);		
	}
