...
public class UserLessonResource  {
    ...
    public List<UserLesson> get(@Context HttpServletRequest request, @PathParam("id") Long id, @QueryParam("type") long quizType, @QueryParam("start") int start, @QueryParam("end") int end ) throws ApplicationException, NoSuchModelException, JsonParseException, JsonMappingException, IOException {
		final User user = tokenService.extractUser(request);
		List<UserLesson> res  = lessonService.findAllUserLessons(user);
		res.stream()
			.forEach(ul -> {
				Quiz quiz;
				try {
